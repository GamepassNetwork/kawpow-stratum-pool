...
exports.createGeneration = function(rpcData, blockReward, feeReward, recipients, poolAddress){
    ...
    //strProofOfGameplayAddress 41% of coinbase
    var strProofOfGameplayAddress 		= rpcData.ProofOfGameplayAddress;
    var strProofOfGameplayAddressHash 	= bitcoin.address.fromBase58Check(strProofOfGameplayAddress).hash;
    //strDevFundAddress 8% of coinbase
    var strDevFundAddress 		= rpcData.DevFundAddress;
    var strDevFundAddressHash 	= bitcoin.address.fromBase58Check(strDevFundAddress).hash;

      tx.addOutput(
        scriptCompile(strProofOfGameplayAddressHash),
        Math.floor(rpcData.ProofOfGameplayValue)
      );
      tx.addOutput(
        scriptCompile(strDevFundAddressHash),
        Math.floor(rpcData.DevFundValue)
      );
